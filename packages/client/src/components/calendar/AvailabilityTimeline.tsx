        ...
        <div className="text-sm">
          {blocks.length === 0 ? (
            <span className="text-green-600 font-medium">
              ✓ {t("calendar.availableNow")}
            </span>
          ) : nextAvailable ? (
            <span className="text-amber-600">
              {t("calendar.nextAvailable", {
                date: format(nextAvailable, "MMM d"),
              })}
            </span>
          ) : (
            <span className="text-red-600">
              {t("calendar.fullyBooked", { days: daysAhead })}
            </span>
          )}
        </div>
      );
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between text-sm text-gray-600">
          ...
          <span className="text-xs">
            {t("calendar.nextDays", { days: daysAhead })}
          </span>
          ...
          {blocks.map((block, index) => {
            ...
            return (
              <div
                ...
                title={`${block.type === "loan" ? t("calendar.legend.checkedOut") : t("calendar.legend.reserved")}: ${format(
                  block.start,
                  "MMM d",
